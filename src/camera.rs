...
pub struct CameraRigFollow;

fn camera_rig_follow(
    query_self: Query<&mut Transform, With<CameraRig>>,
    mut query_follow: Query<&mut Transform, (With<CameraRigFollow>, Changed<Transform>)>
) {
    for _qf in query_follow.iter_mut() {
        todo!("Make the camera follow any Entitiy with CameraRigFollow component")
    }
}